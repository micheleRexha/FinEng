...
optionsToFit<-optionsToFit[optionsToFit$Term == 48, ]
...
returnImplDensity = function(r) {return(returnImplDensityUnscaled(r)/integrate(returnImplDensityUnscaled, lower=-5,upper=5)$value)}

curve(returnImplDensity, from=-1, to=1, xlab="R", ylab="Implied Density")

mean<-integrate(function(x) {return(x*returnImplDensity(x))}, -5, 5)$value
var<-integrate(function(x) {return((x-mean)^2*returnImplDensity(x))}, -5, 5)$value
